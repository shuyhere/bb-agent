...
pub type SharedRequestMetricsState = Arc<Mutex<RequestMetricsState>>;

pub fn new_shared_request_metrics_state() -> SharedRequestMetricsState {
    Arc::new(Mutex::new(RequestMetricsState::default()))
}

pub async fn hydrate_request_metrics_state_from_messages(
...
pub async fn hydrate_request_metrics_state_from_session_messages(
    state: &SharedRequestMetricsState,
    system_prompt: &str,
    tool_defs: &[Value],
...
pub async fn prepare_request_metrics(
    state: &SharedRequestMetricsState,
    request: &CompletionRequest,
...
pub async fn commit_request_metrics_state(
    state: &SharedRequestMetricsState,
...
pub fn append_request_metrics_log(metrics: &RequestCacheMetrics) -> anyhow::Result<()> {
...
mod tests {
    ...
    fn parity_test_request(messages: Vec<Value>, tools: Vec<Value>) -> CompletionRequest {
        CompletionRequest {
    ...
    fn collected_response_with_source(
    ...
    #[tokio::test]
    async fn estimated_metrics_stay_close_to_official_for_repeated_identical_request() {
        let request = parity_test_request(
            vec![json!({"role": "user", "content": "summarize the cache warmup plan"})],
            vec![],
        );
        let state = new_shared_request_metrics_state();
        hydrate_request_metrics_state_from_messages(&state, &request)
            .await
            .expect("seed state");

        let prepared = prepare_request_metrics(&state, &request)
            .await
            .expect("prepare repeated request");
        ...
        let official_usage = resolve_cache_usage(
            &prepared,
            &collected_response_with_source(
                prompt_token_total,
                official_read,
                24,
    ...
    #[tokio::test]
    async fn estimated_metrics_stay_close_to_official_with_tools_and_history() {
        let request = parity_test_request(
            vec![
