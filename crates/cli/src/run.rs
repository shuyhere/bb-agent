...
/// Non-interactive print mode: process a single prompt and exit.
pub async fn run_print_mode(cli: Cli) -> Result<()> {
    ...
    if cli.messages.is_empty() {
        eprintln!("Error: print mode requires a prompt. Usage: bb -p \"your prompt\"");
        return Ok(());
    }
    ...
    loop {
        ...
        for tc in &collected.tool_calls {
            ...
            let (content, is_error) = match result {
                Ok(r) => {
                    eprintln!("  {} {}", "✓".with(Color::Green), tc.name.clone().with(Color::Cyan));
                    (r.content, r.is_error)
                }
                Err(e) => {
                    eprintln!("  {} {}", "✗".with(Color::Red), tc.name.clone().with(Color::Cyan));
...
// ── Utility functions (shared with interactive.rs) ───────────────────
