                ...
                tokio::select! {
                    maybe_submission = submission_rx.recv() => {
                        match maybe_submission {
                            Some(bb_tui::tui::TuiSubmission::CancelLocalAction) => {
                                cancel.cancel();
                                break TreeSummaryAction::Cancelled;
                            }
                            Some(bb_tui::tui::TuiSubmission::Input(_))
                            | Some(bb_tui::tui::TuiSubmission::InputWithImages { .. })
                            | Some(bb_tui::tui::TuiSubmission::MenuSelection { .. })
                            | Some(bb_tui::tui::TuiSubmission::ApprovalDecision { .. })
                            | Some(bb_tui::tui::TuiSubmission::EditQueuedMessages) => {}
                            None => {
                                cancel.cancel();
